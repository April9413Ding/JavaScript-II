...
let firstNamesAllCaps = runners.map(items => { 
...
let ticketPriceTotal = runners.reduce((accum,num) =>{
...
// Problem 1 print out the name and company of the runner who needs small sized shirts
let smallSizedList = runners.filter(list => { return list.shirt_size == "S";}).map(names => { return `name: ${names.first_name} ${names.last_name} company: ${names.company_name}`});
console.log(smallSizedList);

// Problem 2 print out all the donation of the runner who needs Medium sized shirts
let mediumSizedDonation = runners.filter(list => { return list.shirt_size == "M";}).reduce((accum,num) => { return accum+num.donation},0);
console.log(mediumSizedDonation);

// Problem 3 print out the name of top 3 donations

let DonationNum = runners.map(donate => { return donate.donation;}).filter(num => {return num>200;}).sort();

let top3Donation = runners.filter(items => {return items.donation == DonationNum[DonationNum.length-1]||items.donation == DonationNum[DonationNum.length-2]||items.donation == DonationNum[DonationNum.length-3];}).map(name => { return `${name.first_name} ${name.last_name}`})

console.log(top3Donation);